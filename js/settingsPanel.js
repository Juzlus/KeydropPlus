$( document ).ready(async() => {
    ...
    if(config?.hideLiveDrop) {
        ...
        $('div.flex.flex-col.overflow-hidden.bg-darknavy-200.lg\\:flex-row')?.eq(0)?.css('display', 'none');
    }
    $('#hideLiveDrop')?.on('change', function(e) {
        config.hideLiveDrop = e?.target?.checked;
        try { chrome.storage.sync.set({ config: config }); } catch(e) {};
        $('div.flex.flex-col.overflow-hidden.bg-darknavy-200.lg\\:flex-row')?.eq(0)?.css('display', e?.target?.checked ? 'none' : 'flex');
    });
