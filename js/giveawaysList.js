...
const refreshGiveawaysPanel = async(panelText) => {
  ...
  giveawaysHistory?.forEach(el => {
    ...
    tableContent += `<tr class="odd:bg-navy-800"><td class="px-2 py-4 text-center"><a href='https://key-drop.com/giveaways/keydrop/${el?.id}'>${el?.Iwined ? '👑 ' : ''}${el?.id}</a></td><td class="px-2 py-4 text-center">${(sumPrizesPrice * currentRate).toFixed(2) || 0} ${currency || 'USD'}</td><td class="px-2 py-4 text-center">${el?.haveIJoined ? '✔️'  : el?.captcha ? 'reCAPTCHA' : '❌'}</td><td class="px-2 py-4 text-center">${new Date(el?.deadlineTimestamp).toLocaleString()}</td></tr>`;
...
waitForElm('#main-view div.min-h-screen.pt-4.pb-16 div.container').then(() => {
