const createStatisticsButton = async() => {
    ...
    $(`#main-view div.pt-4.pb-16.text-white nav a[href="${window?.location?.pathname?.slice(0, 4)}panel/profil/free-gold"]`)?.eq(0)
        ?.after($(document.createElement('a'))
            ?.addClass('relative flex justify-center whitespace-nowrap bg-navy-700 px-5 text-white opacity-90 transition-colors duration-200 hover:opacity-100 focus-visible:ring-2 focus-visible:ring-gold')
